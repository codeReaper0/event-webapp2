        ...
        <div key={index} className={`py-6 px-4 rounded-2xl ${item.bg}`}>
            ...
            <div className="relative mt-4 flex justify-between gap-3">
                <span className='text-black font-sans font-medium text-base'>
                    <h2 className='font-sans text-xl font-bold text-[#3F3849]'>{item.name}</h2>
                    <h6 className="mt-3">{item.date}</h6>
                    <p className="mt-3 opacity-70">{item.time}</p>
                    <p className="mt-3 font-normal">{item.stadium}</p>
                </span>
                <button className='z-10 active:scale-[0.95]' ><NextBtnSVG /></button>
                <div className='absolute top-[-25px] right-[-16px]'><GroupRectangleSVG /></div>
            </div>
        </div>
    ))

    return (
        <div className='w-full h-[87vh] py-8 px-14 overflow-auto'>
            {/* Header */}
            <div className="relative bg-[url('/Rectangle27.png')] bg-cover bg-center min-h-[300px]">
                ...
                <div className="p-12">
                    <div className="max-w-[420px] w-full font-sans relative z-10">
                        ...
                        <p className="text-[#FFFFFF80] text-base font-medium">Craft events that reflect your passions and interests.</p>
            ...
            <div className="mx-auto mt-8 p-6 bg-[#F0F0F0] rounded-2xl">
                ...
                <div className="flex justify-between items-center gap-10">
                    <span className="flex items-center gap-10 p-4 text-xl font-sans">
                        <button className={`${active === 'friends' ? 'font-bold text-[#3F3849] underline' : 'font-medium text-[#84838B]'} transition-all`} onClick={() => setActive('friends')}>Friends</button>
                        <button className={`${active === 'everyone' ? 'font-bold text-[#3F3849] underline' : 'font-medium text-[#84838B]'} transition-all`} onClick={() => setActive('everyone')}>Everyone</button>
                    </span>
                ...
                <div className="mt-[19px] grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8 ">
                    {pitchData}
                </div>
            </div>
        </div>
